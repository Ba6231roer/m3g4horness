...
1. a1 prepare(Bash,确定性):
     py .opencode/mgh-core/scripts/prepare_augment.py --change <change> [--rules <path>] [--focus <inline-json|path>] [--sensitive-catalog <inline-json|@path|->] [--dry-run] [--no-interactive]
   → stdout = 结构化 change_context.json;产物落 <change-root>/.mgh-sra/change_context.json
   · 读该 stdout 取:`pending[]`、`clarify_path`、`candidate_controls`、`memory`、`project_root`、`focus`、`sensitive_catalog`(**NEVER** `py -c` 重挖)。
