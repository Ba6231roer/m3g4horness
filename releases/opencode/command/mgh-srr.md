...
1. r1 ingest(Bash,确定性):
     py .opencode/mgh-core/scripts/ingest_requirements.py --doc <path|dir|-> [--text <str>] [--rules <path>] [--split] [--out <dir>] [--dry-run] [--no-interactive]
   → stdout = 结构化 change_context.json;产物落 <out-dir>/change_context.json
   · 读该 stdout 取:`pending[]`、`clarify_path`、`candidate_controls`、`memory`、`project_root`、`requirements[]`、`degraded`(**NEVER** `py -c` 重挖)
