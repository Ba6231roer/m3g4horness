...
1. sdr_context(Bash,确定性;外部仓检索在此步、宿主进程内——**可能一次权限确认,诚实披露**):
     py .claude/mgh-core/scripts/sdr_context.py --repo <abs> --run-dir <abs-run-dir> [--base <ref>] [--branch <ref>] [--dimensions <json>] [--no-codegraph]
...
3. fan-out(dispatcher;逐字转发调用,**NEVER** 手工循环):
   ...
   · **快败风暴三层(配额限流形态)**:① `stalled:true` 且
     `py .claude/mgh-core/scripts/diff_group.py --check <run-dir>` 退出码 0(分组产物完好:
...
恢复**必须带同一个 run 目录**(新建目录 = 已完成单元的 marker 不在其中 = 全部重做):
     ...
     py .claude/mgh-core/scripts/resume_sdr_state.py --run-dir <abs-run-dir>
...
(`step ∈ group|fanout|render`)哨兵却缺失 = 守卫休眠 → `--check` 退出码 2 + re-arm recipe:
     ...
     py .claude/mgh-core/scripts/resume_sdr_state.py --run-dir <abs-run-dir> --check
     py .claude/mgh-core/scripts/resume_sdr_state.py --run-dir <abs-run-dir> --rearm-sentinel
...
py .claude/mgh-core/scripts/resume_sdr_state.py --run-dir <abs-run-dir>
py .claude/mgh-core/scripts/resume_sdr_state.py --run-dir <abs-run-dir> --check
py .claude/mgh-core/scripts/list_sdr_steps.py --step fanout
```
