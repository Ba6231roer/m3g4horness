...
**经验 8 — 配额限流网关下,秒级快败风暴比挂死更烧钱,要冷却 + 截断 + 有界重派三层。**
挂死(经验 1–5)烧的是**等待时间**;配额限流(如每 10 分钟 100 次)烧的是**调用数**:429 后
opencode 只秒级重试 2 次(源码核实 `retries: 2`),子代理秒级退出 → runner 零延迟回队重派 →
再撞配额——一轮风暴 1–2 分钟,远快于 10 分钟的配额窗口恢复,整晚耗在「风暴 → 熔断 → resume →
再风暴」。对策三层,快慢互补:

| 层 | 触发 | 动作 | 参数 |
| --- | --- | --- | --- |
| 事件冷却(自愈) | 2 分钟滑动窗内 ≥3 个单元秒级失败回队(crash/timeout/stall/spawn-error;显式 `failed:` ack 不算——它终态化不回队) | 暂停派**新**单元等一段;在飞照常收割;等待按剩余时间预算封顶 | `--cooldown-s`(默认 300,`0`=关,连熔断前退避一起关) |
| 风暴截断(止损) | 自上次磁盘推进起 crash ≥ `--wave` 且输出尾部**全部**命中限流特征(`429`/`too many requests`/`rate limit`/`quota`,大小写不敏感) | 立即停派退出码 2,`rate_limited:true` + 单元清单 + 「等满一个配额窗口再 resume」recipe;优先于新一轮冷却;特征漂移(全 unknown)时不触发、冷却兜底 | `--no-rate-limit-stop` 禁用 |
| 熔断前一次退避 | 零推进熔断将触发且预算允许 | 至多一次「冷却 → 全量重列磁盘 → 再看」:有推进撤销熔断,仍零推进才退出 | 单次调用内至多一次 |

外加一个**有界人工重派口**:显式 `failed:` ack 的单元留终态失败标记、不回队(枚举缺省排除)——
网关瞬断误伤会变成「tier 完成但带覆盖洞」。编排器看 run.log 确认瞬断形态后**至多一次**
`--retry-failed` 重跑:runner 自动携枚举器 `--include-failed`(单元身份永远来自枚举器,
绝不按文件名反推——超长 id 的截断 stem ≠ canonical id)、认领时删 marker 再派,重派再失败
写回新标记,天然无循环。配额环境取值:wave ≈ 配额每分钟上限 × 0.8 ÷ 每单元每分钟呼叫数;
总长表 ≥ 实测单元 p99 × 3–5;**静默表**默认 = 已完成单元 p99 × 2(下限 900 秒)——静默只是
「此刻无产出」不是「卡死」(限流网关下多为排队),可用 `--stall-timeout-s 0` 关掉,派发器还会
按「假杀自证」(被杀单元重派成功)自动放宽本轮生效窗口(`min(2W, call × 0.8)`),每次退出的
`runtime_*` / `stall_killed_slot_pct` 就是重标定的依据(误杀 = 重派 = 丢掉已做部分 + 重新排队)。

**经验 9 — 2026-09-15 真机事故:POSIX 树杀把调用者自己杀了,静默判据又指错了方向。**
独立终端 nohup 直跑 scout 派发器,三次都在**约 10 分钟**猝死(日志无汇总、进度侧车停在
`running`、`--kill-stale` 报无孤儿 = 被强杀、未走正常退出的形状)。三个缺陷叠在失活这一条路上:

| # | 缺陷 | 现象 | 改法 |
| --- | --- | --- | --- |
| ① | 树杀作用域没钉死 | POSIX 树杀用 `os.killpg(os.getpgid(pid))`,而子进程 spawn 时**没建新会话**,与派发器同进程组 → 杀失活单元 = 杀自己;杀发生在写留痕**之前**,连日志都留不下 | spawn 加 `start_new_session`(`setsid`),每个单元自成会话,`killpg` 只覆盖它自己那棵树 |
| ② | 静默判据方向反了 | 两路时间戳初值都是 spawn 时刻,取 `min` 的语义是「**至少一路**静默够久」→ 只要一路**从头到尾没字节**(很多 CLI 全程不写 stderr),健康单元也会在 spawn + 阈值被判失活;600 秒阈值精确对上 10 分钟猝死 | 取 `max`:两路都没新字节才算静默(与 `--help` 的 "no stdout/stderr bytes" 对齐) |
| ③ | 读取粒度不是字节级 | 读子进程输出用 `TextIOWrapper.read(4096)`,它**阻塞到满 4096 字符或 EOF** 才返回 → 「最后输出时刻」实际按 4096 字符块推进。低产出但健康的单元两路合计凑不满 4096 字节,时间戳一直停在 spawn 时刻——②修完照样被误杀 | 改用 `stream.buffer.read1(4096)`(有一次可读即返回)配增量 UTF-8 解码器 |

**①的代价要一起收**:子进程自成会话后,交互式 Ctrl-C 不再随前台进程组抵达它们,必须在派发器的
**退出路径**里显式树杀仍登记的子进程——否则它们会变成「登记已清空、liveness 文件已删除」之后
**谁也扫不到**的孤儿(下次 `--kill-stale` 也查不出来)。硬杀(SIGKILL,退出流程根本没跑)仍留
liveness 文件,由 `--kill-stale` 兜底,这条路径故意不变。

**教训**:① 「杀进程树」在 POSIX 是**进程组**操作,不显式隔离会话,就等于把调用者装进同一颗子弹里;
② 判据取 `min` 还是 `max` 之前,先问清「初值代表什么」——两个流都初始化成 spawn 时刻时,`min` 是
「任一路静默」而非「都静默」;③ 文本模式的 `read(n)` 是「**读满 n**」不是「读现有的」,把这种调用
当事件源会拿到粗一个数量级的粒度(而它的 docstring 可能还写着 "last byte")。
