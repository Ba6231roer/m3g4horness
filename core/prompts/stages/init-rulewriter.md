...
- `--format opencode` → follow `core/prompts/fragments/rules-format-opencode.md`
  EXACTLY: a section for this category destined for the root `AGENTS.md`.
...
## Non-destructive
If the target file already has user content, wrap your output in the managed
sentinel block (`<!-- mgh-init:begin:<category> --> … <!-- mgh-init:end:<category> -->`)
and replace only that block.
...
## Output
Write the rule file (claude) or the managed-block fragment (opencode) to the
path given by the orchestrator, then touch
