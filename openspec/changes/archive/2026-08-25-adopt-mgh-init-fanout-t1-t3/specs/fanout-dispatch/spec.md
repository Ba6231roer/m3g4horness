# fanout-dispatch Delta: adopt-mgh-init-fanout-t1-t3

dispatcher 从 scout 专用泛化为 tier-aware(`--tier scout|t1|t3`),T1/T3 编排器步采纳
dispatcher-first 调用面;T1 scout 闸门与 T3 `repo` 锚补齐泛化前提。

## RENAMED Requirements

- FROM: `### Requirement: 编排器 scout 步调用面切换与回退`
- TO: `### Requirement: 编排器 fan-out 步调用面切换与回退`
