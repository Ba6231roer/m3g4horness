---
description: TODO stub. After an openspec 'propose', run this to analyse and augment specs/tasks with security-design content. If mgh-init rules exist, the added tasks guide which rules to read. Not yet implemented.
allowed-tools: Read, Glob, Grep, Bash, Agent, Write, Edit
---

# /mgh-sra — openspec 安全设计补充 (TODO)

> **状态:TODO — 尚未实现。** 本文件仅为空命令骨架。

## 预期用途

在 openspec 工作流 **`propose` 之后**运行,对生成的 specs / tasks 做**安全设计
补充分析**:为变更补充安全相关的设计要求与任务条目。

### 与 `/mgh-init` 的协作
若已存在 `/mgh-init` 生成的 rules(项目安全设计索引),则本命令补充的 **tasks**
内容应**显式引导读者去查阅对应 rules**——例如"实现 X 时须遵循 rules 中记录的
`@PreAuthorize` 权限校验封装(见 rules:authz/<file>)"。

### 预期参数(占位,尚未解析)
- `--change <name>` — openspec 变更名(默认取 `openspec/changes/` 下最新)
- `--rules <path>` — `/mgh-init` 产出的 rules(可选;存在则增强 task 引导)

## TODO
在后续变更中实现。当前收到本命令时:打印"未实现"说明 +
参数表,**不消耗 token**。
