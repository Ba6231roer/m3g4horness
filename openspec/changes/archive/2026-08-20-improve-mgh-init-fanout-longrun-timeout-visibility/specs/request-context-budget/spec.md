...
**dispatcher 路径增量**(mgh-init scout 首采):当某 tier 的 fan-out 由确定性 dispatcher 叶脚本
...
(`<init-dir>/fanout_progress.json`,见 `fanout-dispatch` 能力);编排器 SHALL NEVER 为查进度
...
- **WHEN** mgh-init scout 层由 `fanout_runner.py` 驱动,pending 批数超过单页预算(如 500 批)
- **THEN** 编排器对整个 scout 层只发起一次 `Bash` 调用(+`partial:true` 时重派同一命令),不出现
...
- **WHEN** 编排器调用/重派 `fanout_runner.py --time-budget-ms 720000`
...
- **THEN** 进度可从 sidecar 文件 `fanout_progress.json` 直接读取(零 token);编排器不为查进度
