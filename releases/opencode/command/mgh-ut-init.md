...
> 编排器 = 你(宿主 agent):按本提示词,用自身工具(Bash / Agent / Read / Write / Edit)把流水线**跑出来**,而非写成代码——确定性逻辑已在 `classify_tests.py` / `list_test_groups.py` / `assemble_test_rules.py` / `validate_test_rules.py` / `derive_mutators.py` 里,直接 `Bash` 调用即可,无需 `Read` 其源码,也不要另写 `.py` 去包装或重实现。opencode 下 rules tier 每 category 直写**详述文件**(`docs/test-conventions/<cat>.md`,独立 H1 文档),`assemble_test_rules.py` 扫该目录建 `AGENTS.md` 简洁**惰性索引块**(按需加载,见步骤 5)。

> **运行域 + hook**:`install.sh` 向本仓 `.opencode/plugins/` 注入 `tool.execute.before`
> 插件(`block-adhoc-scripts`),在 `/mgh-ut-init` 运行域内拦 `py -c`/`python -c` 内省、**一切脚本扩展名写入**
...
> 哨兵 `out_roots[]`;命中阻断 + stderr recipe 指向 `list_test_groups.py` stdout 的 `checkpoint_path`/`rule_path`)。插件把事件归一化后
...
0. parse + self-check(测试源码树统计超阈值则建议 `--scope` 分模块;**花 token 前**前置建议,见 orchestrator-discipline fragment)
   · **起步**:`Bash: export MGH_UT_INIT_ACTIVE=1`(声明运行域,激活 hook,含子树外 Write/Edit 拦截)
   · **run_config(无状态 resume 意图源)**:起步后、花 token 前,**原子写** `<target>/.mgh-ut-init/run_config.json`
     ...
     `py .opencode/mgh-core/scripts/write_ut_runconfig.py --target <abs target> --format <fmt> [--scope ..] [--skip-consistency] [--uniform-sample ..] [--hetero-sample ..] [--subsplit-threshold ..] [--max-unit-bytes ..] [--orch-budget-bytes ..] [--max-aggregate-bytes ..]`
   ...
   · **步骤图**:`py .opencode/mgh-core/scripts/list_ut_steps.py [--step <id>]`(确切脚本调用行;宿主前缀自动派生)
1. classify(确定性,便宜预处理):
     py .opencode/mgh-core/scripts/classify_tests.py --repo <target> --out <target>/.mgh-ut-init [--scope .. --subsplit-threshold F]
   ...
   · 校验:`py .opencode/mgh-core/scripts/classify_tests.py --check <target>/.mgh-ut-init`(每个文件恰进一组、清单与磁盘一致;退出码 2 → 回退重跑)
2. EXTRACT FAN-OUT — 经确定性脚本枚举 + 样本物化(**禁手搓**;fan-out 单元 = 层组,非逐文件):
   [test_groups.json::groups[]] → list_test_groups.py --tier extract --materialize → [stdout slim pending[](每项 `group_id`/`input_path`/`checkpoint_path`/`done_marker`/`failed_marker`)]
     py .opencode/mgh-core/scripts/list_test_groups.py --tier extract --groups <target>/.mgh-ut-init/test_groups.json --checkpoints <target>/.mgh-ut-init/checkpoints/extract --materialize <target>/.mgh-ut-init/inputs/extract [--sample-uniform N --sample-hetero N]
...
3. synthesize(汇总去重;大仓超 `--max-aggregate-bytes` 则披露 + `--scope` 回退):
   ...
   · 校验:`py .opencode/mgh-core/scripts/validate_test_rules.py --inventory <target>/.mgh-ut-init/test_rules_inventory.json`(每条 category/name/anchor/evidence/provenance/confidence;退出码 2 → 回退重跑)
4. RULES FAN-OUT — 经确定性脚本枚举 + per-category 物化(**禁手挖** inventory / `py -c`):
   [test_rules_inventory.json::rules[].category] → list_test_groups.py --tier rules --materialize → [stdout slim pending[](每项 `category`/`input_path`/`rule_path`/`done_marker`/`failed_marker`)]
     py .opencode/mgh-core/scripts/list_test_groups.py --tier rules --inventory <target>/.mgh-ut-init/test_rules_inventory.json --format <format> --checkpoints <target>/.mgh-ut-init/checkpoints/rules --target <target> --rules-dir <target>/docs/test-conventions --materialize <target>/.mgh-ut-init/inputs/rules
   按 `offset`/`effective_limit` 翻页;per category in page `pending[]`(WITHOUT `.done`/`.failed`;`--resume` 跳过):
     ...
     → 成功则恰好写 `rule_path`(绝对;opencode: **详述文件** `docs/test-conventions/<cat>.md`,独立 H1、中性、无哨兵,**禁**直写 `AGENTS.md`)+ touch `done_marker`;失败回 `failed <原因>` ack → 编排器写 `failed_marker`、不重试不阻断
5. (opencode only) BUILD INDEX + LINT (Bash, deterministic):
     py .opencode/mgh-core/scripts/assemble_test_rules.py --target <target> --format opencode [--rules-dir <target>/docs/test-conventions]
   → 扫 `<rules-dir>/*.md` 详述文件建 `<target>/AGENTS.md` 简洁**惰性索引块**
     `<!-- test-conventions:begin --> … :end -->`(每详述文件一行 `@<rel>` + 按需 Read 指令;
     幂等替换、保留用户内容、迁移旧 `mgh-ut-init:` 块)。opencode 启动整份 `AGENTS.md` 进根上下文,
     故索引保持简洁、正文留在详述文件按需加载
   · lint(fail-loud 退出码 2)= 详述文件正文泄漏:工具内部 token(`mgh-ut-init`/脚本名/`.mgh-ut-init/`)/
     schema 字段(`assert_density`/`uniformity`/`weak_dominated`/`group_id`)/ 过程散文(`归类器子分`/`抽样提炼`/`断言密度`)/
     opencode YAML 围栏(`---`)/ 无源码锚点约定。回 rules 修正该 category 详述文件后重跑 5。
...
7. 收尾(确定性):
   · `py .opencode/mgh-core/scripts/derive_mutators.py --repo <target> --out <target>/.mgh-ut-init`(解析 pitest 配置
     → default_mutators.json;无配置 → `source:"builtin-fallback"` 内置标准集 + 披露)
     · 校验:`py .opencode/mgh-core/scripts/derive_mutators.py --check <target>/.mgh-ut-init`
...
```bash
py .opencode/mgh-core/scripts/write_ut_runconfig.py --target . --format opencode
py .opencode/mgh-core/scripts/resume_ut_init_state.py --target .
py .opencode/mgh-core/scripts/resume_ut_init_state.py --target . --check
py .opencode/mgh-core/scripts/list_ut_steps.py --target .
py .opencode/mgh-core/scripts/classify_tests.py --repo . --out ./.mgh-ut-init
py .opencode/mgh-core/scripts/classify_tests.py --check ./.mgh-ut-init
py .opencode/mgh-core/scripts/list_test_groups.py --tier extract --groups ./.mgh-ut-init/test_groups.json --checkpoints ./.mgh-ut-init/checkpoints/extract --materialize ./.mgh-ut-init/inputs/extract --offset 0 --limit 50 --max-unit-bytes 196608 --orch-budget-bytes 65536
py .opencode/mgh-core/scripts/list_test_groups.py --tier rules --inventory ./.mgh-ut-init/test_rules_inventory.json --format opencode --checkpoints ./.mgh-ut-init/checkpoints/rules --target . --rules-dir docs/test-conventions --materialize ./.mgh-ut-init/inputs/rules --offset 0 --limit 50 --max-unit-bytes 196608 --orch-budget-bytes 65536
py .opencode/mgh-core/scripts/validate_test_rules.py --inventory ./.mgh-ut-init/test_rules_inventory.json
py .opencode/mgh-core/scripts/assemble_test_rules.py --target . --format opencode
py .opencode/mgh-core/scripts/assemble_test_rules.py --target . --format opencode --check
py .opencode/mgh-core/scripts/derive_mutators.py --repo . --out ./.mgh-ut-init
py .opencode/mgh-core/scripts/derive_mutators.py --check ./.mgh-ut-init
```

### Resume / cache
- **`--resume` 首步** = `py .opencode/mgh-core/scripts/resume_ut_init_state.py --target <target>` → 读 stdout `step`/`next_action`/`tiers` 继续(进度纯从磁盘 `<target>/.mgh-ut-init/` 重派生;**NEVER** 靠对话记忆判步骤)。`--check` 可在起步校验磁盘状态自洽(退出码 2 = 不自洽)。
...
- rules → opencode:`<target>/AGENTS.md` 简洁**惰性索引块** `<!-- test-conventions:begin --> … :end -->` + 每实现 category 一个详述文件 `<target>/docs/test-conventions/<cat>.md`(由 `assemble_test_rules.py` 扫 `<rules-dir>/*.md` 建索引;详述文件按需加载);claude:`<target>/.claude/rules/test-*.md`
...
- **从目标项目根调用 `/mgh-ut-init`**(launch-cwd 前置):step 0 首调 `list_ut_steps.py` 用相对 `.opencode/mgh-core/scripts/` 路径,解析于编排器 Bash cwd(命令壳加载处 = 目标项目)。下游工具路径经其 stdout `script_abs` 已全钉死绝对,但**首调本身**依赖从目标项目根发起。
