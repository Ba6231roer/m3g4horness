...
> **零打断优选入口**:存量设计引用外部前端仓时,首选 `py .opencode/mgh-core/scripts/mgh_sdr_launch.py`
...
2. diff_group(Bash,确定性;pending 唯一来源):
     py .opencode/mgh-core/scripts/diff_group.py --repo <abs> --base <ref> --branch <ref> --checkpoints <run-dir>/markers --materialize <run-dir>/slices
...
3. fan-out(dispatcher;逐字转发调用,**NEVER** 手工循环):
     py .opencode/mgh-core/scripts/fanout_runner.py --tier sdr --repo <abs> --base <ref> [--branch <ref>] --checkpoints <run-dir>/markers --inputs-dir <run-dir>/slices --time-budget-ms <宿主标定> [--resume]
...
4. render(Bash,确定性):
   ...
   · 校验:`py .opencode/mgh-core/scripts/render_sdr_report.py --check <run-dir>`。
...
| per-unit 复核扇出 | dispatcher `fanout_runner --tier sdr` | `core/prompts/fragments/fanout/sdr-task.md` + agent `sdr-review-fanout` |
...
```bash
py .opencode/mgh-core/scripts/sdr_context.py --repo <abs> --run-dir .mgh-sdr/runs/<ts> --base master
py .opencode/mgh-core/scripts/diff_group.py --repo <abs> --base master --branch feature-pay --checkpoints .mgh-sdr/runs/<ts>/markers --materialize .mgh-sdr/runs/<ts>/slices
py .opencode/mgh-core/scripts/diff_group.py --check .mgh-sdr/runs/<ts>
py .opencode/mgh-core/scripts/fanout_runner.py --tier sdr --repo <abs> --base master --checkpoints .mgh-sdr/runs/<ts>/markers --inputs-dir .mgh-sdr/runs/<ts>/slices --time-budget-ms 720000
