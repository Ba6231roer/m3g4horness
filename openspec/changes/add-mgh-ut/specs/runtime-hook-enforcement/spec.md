...
跨六命令(`/mgh-init`|`/mgh-sast`|`/mgh-sra`|`/mgh-srr`|`/mgh-ut-init`|`/mgh-ut`)共享的运行时纪律守卫
契约——激活模型(env 或磁盘哨兵)、运行域脚本只读、脚本扩展名集、写入受信子树限定。取代此前分散在各
命令 spec、措辞漂移的重复 hook 要求,作为 `block_adhoc_scripts` 守卫(双端 byte-identical)的单一真相源。
本能力由 change `harden-mgh-opencode-hook-enforcement` 建立、`add-mgh-ut-init` 扩到第 5 域
`mgh-ut-init`、`add-mgh-ut` 扩到第 6 域 `mgh-ut`。
...
`MGH_{INIT,SAST,SRA,SRR,UT_INIT,UT}_ACTIVE=1` is set, **OR** (b) a disk sentinel `<run-root>/.active` exists
relative to the guard's cwd, where `<run-root>` is the domain's run directory: `mgh-init`→`.mgh-init/`,
`mgh-sast`→`security-scan/`, `mgh-sra`→`.mgh-sra/`, `mgh-srr`→`.mgh-srr/`, `mgh-ut-init`→`.mgh-ut-init/`,
`mgh-ut`→`.mgh-ut/`. The sentinel SHALL be JSON
...
#### Scenario: mgh-ut activates via env or sentinel as the sixth domain
- **WHEN** `MGH_UT_ACTIVE=1` env 已设,**或** `<cwd>/.mgh-ut/.active` 哨兵存在
- **THEN** 守卫在 `mgh-ut` 运行域激活(第 6 域,与既有 5 域同机制:`py -c` 内省/脚本扩展名写/越权子树写均 fail-loud);哨兵 `domain:"mgh-ut"`、run-root `.mgh-ut`
...
artifacts: checkpoints, inputs, rules, proposals, manifest, report). Leaf scripts under `mgh-core/scripts/` (and
...
`MGH_TARGET` tree (e.g. a drive root, `%LocalAppData%\Temp`, another project dir) — for all six run-domains.
...
`mgh-srr`/`mgh-ut` retain the out-of-tree check **without** the positive allowlist (no root-pollution reported for
those domains; their output paths are already narrowed to `security-scan/`, `.mgh-sra/`, `.mgh-srr/`, `.mgh-ut/`
— `mgh-ut` writes proposals under `.mgh-ut/proposals/`).
...
#### Scenario: mgh-ut proposal writes under .mgh-ut pass; src/test writes are out-of-scope of confinement
- **WHEN** `mgh-ut` 运行域内 `Write` `<target>/.mgh-ut/proposals/<unit>.proposal.json` 与 `.mgh-ut/resume` 检查点
- **THEN** 守卫放行(`.mgh-ut/` 内,输出路径已收窄);守卫**不**直接放行 `src/test` 写入——`/mgh-ut` 壳
  `NEVER` 直写 `src/test`(proposals 暂存是纪律面,补缺合入 `src/test` 属人/apply 评审后动作,不在运行域内)
