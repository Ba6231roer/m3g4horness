...
解析)运行 `git diff --no-color <base>..<branch>`,取得变更文件清单与 hunk。文件 SHALL 按以下
闭集规则分组:**接口单元**(变更落在疑似接口定义文件——java web 项目按 Spring/Servlet 注解启发式
`@RestController`/`@Controller`/`@RequestMapping`/`@GetMapping`/`@PostMapping`/`@PutMapping`/
`@DeleteMapping`/`@PatchMapping`/`@Path`/`@GET`/`@POST` 等锚定;一组注解方法及其 hunk = 一个接口
单元,单元 id 含文件相对路径 + 接口路由)+ **独立变更单元**(剩余全部文件——非接口 java 文件、
配置、SQL、前端透传等——按目录聚类归并,单元 id = 目录簇)。分组是**启发式非承诺**:接口识别
失败(无注解、非 java web)SHALL 退化为一组独立变更单元,流程不失败。每单元 SHALL 物化一份
**只读 slice 文件**(diff hunk + 有限邻近上下文,含文件相对路径、变更类型 A/M/D/R 标注),落
...
`kind`(`interface`|`standalone`)、`route`(接口单元的路由串,独立单元为空串)、`unit_bytes`;
stdout 另携带 `repo` 锚、`branch`/`base`、`total`/`done`/`failed`、`counts{interface,standalone}`。
退出码 `0/1/2`;stdout=结构化 JSON、stderr=诊断严格分流;`--check <run-dir>` 校验 run 产物自洽
(slice 齐备、pending 路径绝对且在 repo 子树、manifest 字段一致),失败退出 2(承 R5.9)。零变更
diff(两 ref 无差异)SHALL 退出码 0 且 `pending: []` + stdout `empty:true`,不进入 fan-out。

#### Scenario: 接口文件按注解分组为多个接口单元

- **WHEN** 某 diff 触及 `UserController.java`(含 `@RequestMapping("/user")` 类级注解 + 新增
  `@PostMapping("/detail")` 方法 hunk)与 `OrderService.java`(无注解,普通服务类修改)
- **THEN** `diff_group.py --materialize` 产出 1 个 `kind=interface` 单元(id 含 `UserController` +
  路由 `/user/detail`,`route="/user/detail"`,slice 含该方法 hunk)与 1 个 `kind=standalone`
  单元(归并 `OrderService.java` 所在目录簇),各自 `input_path` 绝对且在 repo 子树内

#### Scenario: 无注解项目退化为独立变更单元模式

- **WHEN** diff 触及的文件全部不含任何接口注解启发式命中(如纯 SQL 迁移 + 工具类)
- **THEN** 全部文件按目录聚类归并为若干 `kind=standalone` 单元,流程退出码 0,pending 非空,
  不报错不失败
