...
{"capability":"<cap>",
 "gaps":[{"dimension":"horizontal-authz",
   ...
   "risk":"<为何是缺口>","recommended_control":{...}|null,"matched_signals":{...}|null}],
...
| `gaps[].recommended_control` | 仅三信号同时命中时给(`{name,evidence,rule_path}` + 业务域相似理由);无 `--rules` 时 `null` |
| `gaps[].matched_signals` | `{dimension_fit:bool, business_domain:bool, business_fact:bool}`;仅文件重叠不算业务域命中 |
| `security_requirements[]` / `security_tasks[]` | 渲染进 specs / tasks 的受管块;每条锚定 + 「复用勿重造」措辞 |
