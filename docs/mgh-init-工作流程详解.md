...
flowchart TD
    ...
    foldin --> t1e["取待跑簇清单<br/>list_clusters.py"]
...
| --------- | ----------- | ------------------------------ | ----------- | -------------------------------------------------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| i0        | 起步自检        | —(编排器)                         | —           | 无(编排器自身)                                                             | 复用 `discover_controls` 计数                                          | 校验参数(`--format` 必填)、检查子代理模型可用、统计源文件、大仓前置建议、声明运行域                      | 无文件;stderr 进度 + 大仓建议                                                               |
...
| i4        | 收尾          | —(编排器)                         | —           | 无(编排器自身)                                                             | 无                                                                  | 写清单 + 报告,打印产物路径 + 披露项                                                 | `init_manifest.json`、`report.md`                                                   |
...
- **干什么**:每个簇在一个隔离上下文里,由子代理读它的证据文件(+ 分布式形状的几处用法),
  归纳出"这个控制**是什么 / 开发者该怎么复用 / 有什么覆盖缺口**"的一条结构化记录。
...
| 名词 | 通俗解释 |
|---|---|
| **候选 candidate** | 一个"疑似安全控制"的发现,带分类、锚点、证据片段、置信度、来源(`regex`/`scout`) |
...
| 文件 | 内容 | 谁产出 |
|---|---|---|
| `controls_candidates.json` | 全部候选(正则命中 + scout),每条带 `source` | i1 + 3b-foldin |
...
| `init_manifest.json` | 版本 / 格式 / 计数 / provenance / unresolved[] / boundaries[] | i4 |
...
合并多次范围运行后停止;`--no-scout` 关掉语义侦察(回退纯正则);`--skip-consistency` 关掉 T4;
`--no-enforce-hook` 安装时不注入运行时纪律 hook。
