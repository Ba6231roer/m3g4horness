...
0. parse + self-check (host agent/model available; else STOP with fix hint;发现脚本统计源文件数,超 `--large-repo-threshold` 则建议 `--scope`+`--merge`(**花 token 前**前置建议;波次进行中不再因规模打断,见 orchestrator-discipline fragment),扫描期向 stderr 打印进度)
   ...
   · **哨兵(磁盘激活信号,opencode 可靠激活兜底)**:`write_runconfig.py` 写 run_config 的同时**确定性 co-write**
     `<target>/.mgh-init/.active`(副作用,零手写配方):`domain:"mgh-init"`、`target` = 其 stdout 的 `target`
     (**绝对项目根**,Windows 原生、供守卫 `Path.resolve()` 判树;**NEVER** 用 bash `pwd`,其 MSYS `/c/...` 在
     Windows pathlib 误解析)、非默认 `--out`/`--rules-dir` 解析后绝对根入 `out_roots[]`(默认产物根不列)、`v:1`)。
     脚本一跑哨兵必在。守卫激活 = `MGH_INIT_ACTIVE=1` env **或** 该哨兵(opencode 插件进程不继承 mid-session env →
     哨兵兜底,使脚本只读 / 受信子树守卫在 opencode 上整 run 可靠激活)。哨兵携 `target` 使 `MGH_TARGET` 在首 run 即就绪。
     完成态(done 步)/ 干净停止 `rm <target>/.mgh-init/.active`(避免残留哨兵锁死日常开发);`--resume` / 压缩后
     第一步经 `resume_state.py --rearm-sentinel` 确定性重写(据磁盘 `run_config.target` 派生,NEVER 手拼)。
